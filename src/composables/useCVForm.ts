...
export function useCVForm() {
  ...
  function buildCVHTML(): string {
    ...
    const contacts = [form.phone, form.email, form.city, form.linkedin].filter(
      Boolean,
    );
    h +=
    ...
    if (visibleJobs.length) {
      ...
      visibleJobs.forEach((j) => {
        ...
        if (j.from || j.to)
          h +=
            '<span class="cv-job-date">' +
            esc(j.from) +
            (j.from && j.to ? " &ndash; " : "") +
            esc(j.to) +
            "</span>";
        h += "</div>";
        const co = [esc(j.company), esc(j.location)]
          .filter(Boolean)
          .join(" &nbsp;|&nbsp; ");
    ...
    if (visibleEdus.length) {
      ...
      visibleEdus.forEach((e) => {
        ...
        if (e.from || e.to)
          h +=
            '<span class="cv-edu-yr">' +
            esc(e.from) +
            (e.from && e.to ? " &ndash; " : "") +
            esc(e.to) +
            "</span>";
        h += "</div>";
        const sc = [esc(e.school), esc(e.location)]
          .filter(Boolean)
          .join(" &nbsp;|&nbsp; ");
    ...
    if (visibleSkills.length) {
      ...
      visibleSkills.forEach((s) => {
        const lbl =
          esc(s.label) + (s.label && !s.label.trim().endsWith(":") ? ":" : "");
        h +=
          '<div class="cv-skill-row"><span class="cv-skill-lbl">' +
          lbl +
          '</span><span class="cv-skill-val">' +
          esc(s.values) +
          "</span></div>";
      });
    }
    ...
    if (visibleCerts.length) {
      ...
      h +=
        '<div class="cv-cert"><ul>' +
        visibleCerts.map((c) => "<li>" + esc(c) + "</li>").join("") +
        "</ul></div>";
    }
    ...
    if (visibleRefs.length) {
      ...
      visibleRefs.forEach((r) => {
        ...
        if (r.phone)
          h += '<div class="cv-ref-phone">' + esc(r.phone) + "</div>";
  ...
  function scalePreview() {
    ...
    const firstPage = wrap.querySelector(".cv-page") as HTMLElement;
    const pageWidth = firstPage ? firstPage.offsetWidth : 794;
  ...
  const printPDF = () => {
    ...
    openPrintWindow(previewContainer.value, form.name || "CV");
  };
  ...
  const downloadDOCX = async () => {
    try {
      ...
      if (!docxLib) {
        alert(
          "docx library failed to load. Please refresh the page and try again.",
        );
        return;
      }
      ...
      const contacts = [phone, email, city, linkedin]
        .filter(Boolean)
        .join("  |  ");
      if (contacts) {
        sections.push(
      ...
      if (summary.trim()) {
        sections.push(
          new Paragraph({
            ...
            children: [
              createRun("PROFESSIONAL SUMMARY", { bold: true, size: 9 }),
            ],
          }),
        );
        sections.push(
      ...
      if (visibleSkills.length) {
        ...
        visibleSkills.forEach((s) => {
          const lbl =
            s.label + (s.label && !s.label.trim().endsWith(":") ? ":" : "");
          sections.push(
            new Paragraph({
              spacing: { after: 40 },
              children: [
      ...
      if (visibleCerts.length) {
        sections.push(
          new Paragraph({
            ...
            children: [
              createRun("CERTIFICATIONS & TRAINING", { bold: true, size: 9 }),
            ],
          }),
        );
